...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'bbp-core' ) ) );
      } else {
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! $ajax && ! empty( $response[ 'csf_import_data' ] ) ) {
          ...
          $importing    = true;
          $this->notice = esc_html__( 'Settings successfully imported.', 'bbp-core' );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'Default settings restored.', 'bbp-core' );
        ...
        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'Settings saved.', 'bbp-core' );
        }
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'bbp-core' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( 'show all settings', 'bbp-core' ) .'"><i class="fas fa-outdent"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( 'Search...', 'bbp-core' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'bbp-core' ) .'" data-save="'. esc_html__( 'Saving...', 'bbp-core' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'bbp-core' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'bbp-core' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'bbp-core' ) : esc_html__( 'Reset', 'bbp-core' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'bbp-core' ) .'">' : '';
            echo '</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'bbp-core' ) .'" data-save="'. esc_html__( 'Saving...', 'bbp-core' ) .'">';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'bbp-core' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'bbp-core' ) .'">' : '';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'bbp-core' ) : esc_html__( 'Reset', 'bbp-core' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'bbp-core' ) .'">' : '';
          echo '</div>';
