  ...
  class CSF_Shortcoder extends CSF_Abstract{
    ...
    public $args         = array(
      ...
      'gutenberg'        => array(
        ...
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
        'keywords'       => array( 'shortcode', 'bbp-core', 'insert' ),
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'csf_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'bbp-core' );
        ...
        echo '<div class="csf-field csf-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'bbp-core' ) .'</div>';
      }
